...
class ChirpGPU:
    ...
    def __init__(self, chirp_cpu: Chirp):
        self.rate = chirp_cpu.rate
        self.fc = chirp_cpu.fc
        self.duration = chirp_cpu.duration
    ...
    def real_chirp(self, t):
        ...
        rate = self.rate
        duration = self.duration
        return cp.cos(2 * cp.pi * (fc * t + rate * t ** 2 / 2)) * cp.where(cp.abs(t) <= duration / 2, 1, 0)
    ...
    def baseband_chirp(self, t):
        ...
        rate = self.rate
        duration = self.duration
        s = (1 / cp.sqrt(2)) * cp.exp(1j * cp.pi * rate * t ** 2) * cp.where(cp.abs(t) <= duration / 2, 1, 0)
        return s
    ...
    def baseband_chirp_delayed(self, t, delay):
        ...
        duration = self.duration
        fc = self.fc
        s = (1 / cp.sqrt(2)) * cp.exp(-1j * 2 * cp.pi * fc * delay) * \
            cp.exp(1j * cp.pi * rate * (t - delay) ** 2) * \
            cp.where(cp.abs(t - delay) <= duration / 2, 1, 0)
        return s
    ...
    def basebanded_train_jit(t, prf, rate, duration, fc):
        ...
        for n in (prange(int(n_min), int(n_min + n_tot))):
            ...
            impulse_begin_index = (cp.abs(t - impulse_begin_time)).argmin()
            impulse_end_index = (cp.abs(t - impulse_end_time)).argmin()

            t1 = t[impulse_begin_index:impulse_end_index]
            delay = n / prf
            s1 = (1 / cp.sqrt(2)) * cp.exp(-1j * 2 * cp.pi * fc * delay) * \
                 cp.exp(1j * cp.pi * rate * (t1 - delay) ** 2) * \
                 cp.where(cp.abs(t1 - delay) <= duration / 2, 1, 0)
...
# jitted, just a wrapper
    ...
    @staticmethod
    @jit(nopython=True, parallel=True)
    def baseband_chirp_train_delayed_jit(t: cp.ndarray, delay: cp.ndarray, prf, rate, duration, fc):
        # allocate memory for rx signal
        x = 1j * cp.zeros_like(t)
        # maximum delay
        max_delay = max(cp.abs(delay))
        # minumum delay
        min_delay = min(cp.abs(delay))
        # sampling period
        delta_t = t - cp.roll(t, 1)
        Ts = cp.sum(delta_t[1:len(delta_t)])/(len(delta_t)-1)
        ...
        for ii in (prange(len(p_d))): # for every pulse
            # idx = cp.argmin(cp.abs(t - p[ii])) # pulse time axis index
        ...
        for ii in (prange(len(p))):
            ...
            sbb = s = 1 * cp.exp(-1j * 2 * cp.pi * fc * dlay) * \
                cp.exp(1j * cp.pi * rate * (t1 - dlay) ** 2) * \
                cp.where(cp.abs(t1 - dlay) <= duration / 2, 1, 0)
            ...
            sbb *= cp.exp(1j * 2 * cp.pi * p[ii] * fc) # here the demodulation simply appears as a constant phase term
