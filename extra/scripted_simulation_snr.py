...
# %%
# - 5 NOISE SIGNAL GENERATION
# the Idea is that we create a noise signal and run it trough the range_doppler algorithm
noise_power = 1 # W
# create a complex gaussian white noise of the same length of the time axis
noise_raw = np.random.normal(0, noise_power / np.sqrt(2) , len(data.time)) +\
            1j * np.random.normal(0, noise_power / np.sqrt(2) , len(data.time))
# create a new data object to be saved in a new folder
noise_data = Data(directory_path='./Noise_data/')
noise_data.set_prf(radar.prf, fs)
noise_data.time_axis(data.Fs, -t_minmax, t_minmax)
noise_data.set_rx_data(noise_raw)
# dump data content to noise folder
import pickle as pk
print('pickling')
filename = './Noise_Data/data_dump.pk'
os.makedirs(os.path.dirname(filename), exist_ok=True)
with open(filename, 'wb') as handle:
    pk.dump(noise_data, handle)
    handle.close()
# create a new rangedop class and compress all
channel.filter_raw_signal(noise_data)
noise_rangedop = RangeDopplerCompressor(channel, noise_data)
# compress the image
noise_outimage = noise_rangedop.azimuth_compression(doppler_bandwidth = doppler_bandwidth)
