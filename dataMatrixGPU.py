...
class DataGPU:
    ...
    def __init__(self, data_cpu: Data): 
        # directory path
        self.directory_path = data_cpu.directory_path

        ## data storage template

        # parameters
        self.prf = data_cpu.prf
        self.Fs = data_cpu.Fs
        self.rows_num = data_cpu.rows_num
        self.columns_num = data_cpu.columns_num


        # axis (these are filled at need when a getter is called - see getters section)
        self.fast_time_axis = data_cpu.fast_time_axis
        self.slow_time_axis = data_cpu.slow_time_axis
        self.range_axis = data_cpu.range_axis
        self.azimuth_axis = data_cpu.azimuth_axis

        # arrays and matrices (filled at need - see setters section)
        self.time = cp.asarray(data_cpu.time)  # simulation time axis
        self.data = cp.asarray(data_cpu.data)  # raw signal array (not enveloped)
        self.data_matrix = cp.asarray(data_cpu.data_matrix)  # same of data, but enveloped on a 2-d array (slow and fast time)
        self.data_range_matrix = data_cpu.data_range_matrix  # range compressed signal in time domain - 2-d array
        self.doppler_range_compressed_matrix = data_cpu.doppler_range_compressed_matrix  # range compressed signal in the range-doppler domain - 2-d array
        self.doppler_range_compressed_matrix_rcmc = data_cpu.doppler_range_compressed_matrix_rcmc  # range compressed signal in the range-doppler domain after
        ...
        # print(type(self.azimuth_axis))
        # print(type(self.time))
        # print(type(self.data))
        # print(type(self.data_matrix))
        # print(type(self.data_range_matrix))
        # print(type(self.doppler_range_compressed_matrix))
        # print(type(self.doppler_range_compressed_matrix_rcmc))
        # print(type(self.range_doppler_reconstructed_image))
        # print(type(self.reconstructed_image))
    ...
    def set_prf(self, prf, Fs):
        ...
        :return: none
        """
        pri = 1 / Fs * cp.round(Fs / prf)
        self.prf = 1 / pri
        return self.prf

    def set_rx_data(self, data: cp.ndarray):
        """
        ...
        if len(self.data) != len(self.time):
            ...
            return
        matrix_rows = int(cp.round(len(self.data) / (self.Fs / self.prf)))
        self.rows_num = matrix_rows
        matrix_columns = int(cp.round(len(self.data) / matrix_rows))
        self.columns_num = matrix_columns
        self.data_matrix = self.data.reshape((matrix_rows, matrix_columns))
        return self.data_matrix

    def set_range_compressed_data(self, data: cp.ndarray):
        """
        ...
        if len(self.data_range) != len(self.time):
            print("ERROR, data must e computed on the provided time axis")
            return
        matrix_rows = int(cp.round(len(self.data_range) / (self.Fs / self.prf)))
        matrix_columns = int(cp.round(len(self.data_range) / matrix_rows))
        self.data_range_matrix = self.data_range.reshape((matrix_rows, matrix_columns))
        return self.data_range_matrix

    def set_doppler_range_compressed_matrix(self, data: cp.ndarray):
        self.doppler_range_compressed_matrix = data

    def set_doppler_range_compressed_matrix_rcmc(self, data: cp.ndarray):
        self.doppler_range_compressed_matrix_rcmc = data

    def set_range_doppler_reconstructed_image(self, data: cp.ndarray):
        self.range_doppler_reconstructed_image = data

    def set_reconstructed_image(self, data: cp.ndarray):
    ...
    def time_axis(self, Fs, min, max):
        ...
        self.Fs = Fs
        min_ = int(cp.floor(float(min) * self.prf)) / self.prf
        max_ = int(cp.ceil(float(max) * self.prf)) / self.prf
        ...
        if int(((max_ - min_) * self.prf) % 2 ) == 0:
            max_ = int(1 + cp.ceil(float(max) * self.prf)) / self.prf

        self.time = cp.linspace(min_, max_ - 1 / self.Fs, cp.round((max_ - min_) * self.Fs).astype('int'))
    ...
    def get_fast_time_axis(self):
        ...
        self.fast_time_axis = cp.linspace(0, (1 / self.prf) - (1 / self.Fs), int(cp.round(self.Fs / self.prf)))
    ...
    def get_doppler_axis(self, prf, doppler_centroid):
        ...
        doppler_ax = cp.linspace(-prf / 2,
                                 (1 - 1 / len(self.data_matrix[:, 0])) * prf / 2,
                                 len(self.data_matrix[:, 0]))
        f_offset = cp.ceil(doppler_centroid / prf) * prf
        doppler_ax += f_offset
        return doppler_ax
    ...
    def get_range_line(self, index, extra_samples):
        ...
        zero_pad_left = 1j * cp.zeros(int(-jj * (cp.sign(-jj) + 1) / 2))
        jj += len(zero_pad_left)
        # check if it needs zeros after
        kk = ii + len(self.data_matrix[0, :]) + extra_samples
        zero_pad_right = 1j * cp.zeros(int((kk - len(self.data)) *
                                           (cp.sign(kk - len(self.data)) + 1) / 2))
        kk -= len(zero_pad_right)
        # pack everything
        data_out = self.data[jj:kk]
        data_out = cp.concatenate((zero_pad_left, data_out, zero_pad_right))
        return data_out

    ## LOAD AND DUMP DATA FIELDS

    # DUMPERS
    def dump_rx_data(self, data: cp.ndarray = None, dir_path: str = None):
        """
    ...
    def dump_range_compressed_matrix(self, range_compressed_data: cp.ndarray = None, dir_path: str = None):
        """
    ...
    def dump_doppler_range_compressed_matrix(self, doppler_range_compressed_matrix: cp.ndarray = None,
                                             dir_path: str = None):
        """
    ...
    def dump_doppler_range_compressed_matrix_rcmc(self, doppler_range_compressed_matrix_rcmc: cp.ndarray = None,
                                                  dir_path: str = None):
        """
        dump the range-doppler transformed range compressed data matrix, 
        frees the memory allocated for the corresponding attribute after dumping.
    ...
    def dump_range_doppler_reconstructed_image(self, range_doppler_reconstructed_image: cp.ndarray = None,
                                               dir_path: str = None):
        """
    ...
    def dump_reconstructed_image(self, reconstructed_image: cp.ndarray = None, dir_path: str = None):
        """
